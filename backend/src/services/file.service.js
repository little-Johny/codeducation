...
class FileService {
    ...
    async downloadFile(id) {
        try {
            const file = await this.getFileById(id);
            const filePath = path.resolve(process.cwd(), "uploads/files", file.filename);

            console.log("DEBUG: intentando leer archivo en:", filePath);
            ...
            if (error.code === "ENOENT") {
                console.error("Archivo no encontrado físicamente");
