...
class UserLikesService {
    ...
    async likeLesson(userId, lessonId) {
        try {
            ...
            const existingLike = await this.userLikesRepository.findByUserAndLesson(
                userId,
                lessonId
            );
            if (existingLike) {
    ...
    async unlikeLesson(userId, lessonId) {
        const deleted = await this.userLikesRepository.delete(userId, lessonId);
        if (!deleted) throw boom.notFound("Like not found");
        return deleted;
    }

    async deleteAll(userId) {
        return await this.userLikesRepository.deleteAll(userId);
    }
    ...
    async toggleLike(userId, lessonId) {
        ...
        try {
            ...
            if (existingLike) {
                ...
                const newLike = await this.userLikesRepository.create(
                    { userId, lessonId },
                    { transaction }
                );
                await transaction.commit();
